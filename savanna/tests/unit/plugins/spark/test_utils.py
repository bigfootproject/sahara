...
class SparkUtilsTest(unittest2.TestCase):
    def setUp(self):
        ...
        ng1 = _make_ng_dict("master", "f1", ["master", "nn"], 1, [i1])
        ng2 = _make_ng_dict("workers", "f1", ["slave", "dn"], 3,
                            [i2, i3, i4])
        ng3 = _make_ng_dict("sn", "f1", ["dn"], 1, [i5])

        self.c1 = _create_cluster("cluster1", "tenant1", "spark",
                                  "2.0.0-cdh4.4.0",
                                  [ng1, ng2, ng3])
    ...
    def test_get_node_groups(self):
        ...
        self.assertListEqual(u.get_node_groups(self.c1, ['dn', 'slave']),
                             [self.ng2])
        self.assertListEqual(u.get_node_groups(self.c1, 'dn'),
                             [self.ng2, self.ng3])
        self.assertListEqual(u.get_node_groups(self.c1, ['dn']),
                             [self.ng2, self.ng3])
        self.assertListEqual(u.get_node_groups(self.c1, ['master', 'slave']),
                             [])
